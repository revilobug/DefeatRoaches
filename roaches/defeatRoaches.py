...
class ZergAgent(base_agent.BaseAgent):
    def __init__(self):
        ...
        if fn != "":
            ...
            self.label = tf.keras.backend.placeholder(dtype = tf.int32, shape = [None,], name = 'actions')
            self.G = tf.keras.backend.placeholder(dtype = tf.float32, shape = [None,], name = 'G')
            ...
            self.action_model = tf.keras.models.Sequential(
                [
                    ...
                    layers.Dense(
                        self.action_space, activation="linear", name = "output"
                        ),
                ]
            )

            test = self.action_model.layers[3]

            self.action_model.compile(
                loss=tf.nn.softmax_cross_entropy_with_logits(labels = self.label, logits = tf.convert_to_tensor(self.action_model.layers[3])) * self.G,
            ...
            test = self.location_model.layers[3]

            self.location_model.compile(
                loss=tf.nn.softmax_cross_entropy_with_logits(labels = self.label, logits = self.location_model.layers[3]) * self.G,
                # loss=tf.nn.sigmoid_cross_entropy_with_logits(),
                # loss="categorical_crossentropy",
                optimizer=tf.keras.optimizers.Adam(0.0001),
            )            
    ...
    def NN_step(self, obs):
        # NN outputs softmax
        # outputs [no_op/0, select_army/7, Attack_screen/12]
        ...
        if currentAction == 0:
            ...
            action = FUNCTIONS.Attack_screen("now", [NNx, NNy])
            currentActionID = 12
    ...
    def scripted_step(self, obs):
        if FUNCTIONS.Attack_screen.id in obs.observation.available_actions:
            ...
            roaches = _xy_locs(player_relative == PLAYER_ENEMY)
            if not roaches:
    ...
    def remember (self, curr_obs, action, reward, next_obs):
        ...
        if not roaches:
            ...
            roachesLocation = np.mean(roaches, axis=0).round()
        ...
        self.memory.append([state, flatAction, roachesLocation, reward, next_state])
    ...
    def learn(self):
        ...
        # #[state, flatAction, beaconLocation, reward, next_state]
        ...
        states_memory = np.array(self.memory[0])
        action_memory = np.array(self.memory[1])
        location_memory = np.array(self.memory[2])
        reward_memory = np.array(self.memory[3])
        ...
        self.action_model.fit(
            feed_dict = {self.observation_space: states_memory, 
